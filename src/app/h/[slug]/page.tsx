    ...
    <div className="flex min-h-screen flex-col">
      ...
      <main className="flex-1 px-4 py-4">
        <div className="mb-6">
          ...
          {hotel.description && (
            <p className="mt-2 text-sm text-black/70 dark:text-white/70">{hotel.description}</p>
          )}
        </div>
        ...
        >
          ...
          <button
            type="submit"
            className="rounded-xl bg-slate-900 px-4 py-3 text-sm font-semibold text-white dark:bg-white dark:text-slate-900"
          >
        ...
        <div className="flex flex-col gap-4">
          {roomTypes.map((rt) => {
            ...
            return (
              ...
              >
                {photo ? (
                  <div className="relative h-40 w-full bg-black/5 dark:bg-white/10">
                    <Image src={photo} alt={rt.name} fill unoptimized className="object-cover" />
                  </div>
                ) : (
                  <div className="flex h-40 w-full items-center justify-center bg-black/5 text-3xl font-semibold text-black/30 dark:bg-white/10 dark:text-white/30">
                    {rt.name[0]}
                  </div>
                )}
                <div className="flex flex-col gap-2 p-4">
                  ...
                  {rt.description && (
                    <p className="text-sm text-black/70 dark:text-white/70">{rt.description}</p>
                  )}
